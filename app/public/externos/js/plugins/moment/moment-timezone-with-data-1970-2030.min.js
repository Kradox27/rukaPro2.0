! function(c, M) { "use strict"; "object" == typeof module && module.exports ? module.exports = M(require("moment")) : "function" == typeof define && define.amd ? define(["moment"], M) : M(c.moment) }(this, function(b) {
    "use strict";
    ...
    function r(c) { return 96 < c ? c - 87 : 64 < c ? c - 29 : c - 48 }
    ...
    function O(c) { for (var M = 0; M < c.length; M++) c[M] = o(c[M]) }

    function q(c, M) { var a, A = []; for (a = 0; a < M.length; a++) A[a] = c[M[a]]; return A }
    ...
    function d(c) {
        ...
        return O(a), O(A), O(o),
            function(c, M) {
                ...
                c[M - 1] = 1 / 0
            }(o, A.length), { name: M[0], abbrs: q(M[1].split(" "), A), offsets: q(a, A), untils: o, population: 0 | M[5] }
    }

    function f(c) { c && this._set(d(c)) }

    function N(c, M) { this.name = c, this.zones = M }
    ...
    function l(c) { this.zone = c, this.offsetScore = 0, this.abbrScore = 0 }

    function L(c, M) { for (var a, A; A = 6e4 * ((M.at - c.at) / 12e4 | 0);)(a = new t(new Date(c.at + A))).offset === c.offset ? c = a : M = a; return c }

    function u(c, M) { return c.offsetScore !== M.offsetScore ? c.offsetScore - M.offsetScore : c.abbrScore !== M.abbrScore ? c.abbrScore - M.abbrScore : c.zone.population !== M.zone.population ? M.zone.population - c.zone.population : M.zone.name.localeCompare(c.zone.name) }

    function W(c, M) { var a, A; for (O(M), a = 0; a < M.length; a++) A = M[a], e[A] = e[A] || {}, e[A][c] = !0 }
    ...
    function B() {
        ...
        var a, A, o, b = function() {
                var c, M, a, A = (new Date).getFullYear() - 2,
            ...
            p = b.length,
            i = function(c) {
                var M, a, A, o = c.length,
                    b = {},
                    p = [];
                for (M = 0; M < o; M++)
                    for (a in A = e[c[M].offset] || {}) A.hasOwnProperty(a) && (b[a] = !0);
    ...
    function X(c) { return (c || "").toLowerCase().replace(/\//g, "_") }

    function s(c) { var M, a, A, o; for ("string" == typeof c && (c = [c]), M = 0; M < c.length; M++) o = X(a = (A = c[M].split("|"))[0]), p[o] = c[M], z[o] = a, W(o, A[2].split(" ")) }

    function m(c, M) { c = X(c); var a, A = p[c]; return A instanceof f ? A : "string" == typeof A ? (A = new f(A), p[c] = A) : i[c] && M !== m && (a = m(i[c], m)) ? ((A = p[c] = new f)._set(a), A.name = z[c], A) : null }

    function T(c) { var M, a, A, o; for ("string" == typeof c && (c = [c]), M = 0; M < c.length; M++) A = X((a = c[M].split("|"))[0]), o = X(a[1]), i[A] = o, z[A] = a[0], i[o] = A, z[o] = a[1] }
    ...
    function E(c) {
        s(c.zones), T(c.links),
            function(c) {
                var M, a, A, o;
                if (c && c.length)
                    for (M = 0; M < c.length; M++) a = (o = c[M].split("|"))[0].toUpperCase(), A = o[1].split(" "), n[a] = new N(a, A)
            }(c.countries), C.dataVersion = c.version
    }

    function R(c) { var M = "X" === c._f || "x" === c._f; return !(!c._a || void 0 !== c._tzm || M) }

    function S(c) { "undefined" != typeof console && "function" == typeof console.error && console.error(c) }
    ...
    function C(c) {
        ...
        _set: function(c) { this.name = c.name, this.abbrs = c.abbrs, this.untils = c.untils, this.offsets = c.offsets, this.population = c.population },
        _index: function(c) {
            var M, a = +c,
                A = this.untils;
        ...
        countries: function() { var M = this.name; return Object.keys(n).filter(function(c) { return -1 !== n[c].zones.indexOf(M) }) },
        parse: function(c) {
            var M, a, A, o, b = +c,
                p = this.offsets,
                i = this.untils,
